...
const About = () => {
  ...
  return (
    <div className='overflow-hidden'>
      <section className="relative bg-black overflow-hidden h-[220px] sm:h-[350px] md:h-[450px] lg:h-[600px] transition-colors duration-300">
        {/* Background Hyperspeed Dark */}
      ...
      <section className='bg-white dark:bg-black sm:pt-[70px] pt-[50px] pb-[50px] overflow-hidden transition-colors duration-300'>
        <div className='container mx-auto'>
          <div className='text-center'>
            <h2 className="text-black dark:text-white text-[22px] sm:text-[32px] md:text-[40px] lg:text-[42px] sx:leading-[35px] leading-[28px]">
              Explore our <span className='text-themedark'>research</span>
            </h2>
            <p className=" text-gray-700 dark:text-[#9aa0a6] text-[12px] sm:text-[14px] md:text-[16px] mt-4">
      ...
      <section className='bg-white dark:bg-black overflow-hidden transition-colors duration-300'>
        <div className='sm:pt-[70px] pt-[40px]'>
          <div className="container mx-auto">
            <div className='text-black dark:text-white text-center'>
              <p>People and culture</p>
              <h3 className="text-black dark:text-white text-[22px] sm:text-[32px] md:text-[40px] lg:text-[40px] sm:leading-[48px] sx:leading-[28px] leading-[28px] mt-4">The liberty to  <span className='text-themedark'>develop, test, </span> <br /> and <span className='text-themedark'>expand AI </span> solutions </h3>
                      ...
                      {accordionData.map((item, index) => {
                        ...
                        return (
                          <div key={index} data-aos="fade-right">
                            {/* Header */}
                            <button onClick={() => toggle(index)} className="flex w-full lg:my-8 my-4 justify-between text-black dark:text-white sm:text-[20px] text-[18px] items-center text-left font-semibold text-lg focus:outline-none">
                              {item.title}
                              {isOpen ? (
                                <BiMinus className="w-5 h-5 border-[1px] rounded-full text-black dark:text-white border-black dark:border-white" />
                              ) : (
                                <BiPlus className="w-5 h-5 border-[1px] rounded-full text-black dark:text-white border-black dark:border-white" />
                              )}
                            </button>

                            {/* Content with transition */}
                            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
                              <p className="mt-2 text-gray-700 dark:text-white mb-7">{item.content}</p>
                            </div>
                            <div className="border-t border-[#7e7d7d]"></div>
                          </div>
                        );
                ...
                <div className="flex lg:flex-nowrap lg:flex-row flex-col-reverse flex-wrap items-center">
                  ...
                  <div className="lg:w-1/2 w-full lg:px-[50px] px-5">
                    <div className='text-black dark:text-white'>
                      <h3 className='text-black dark:text-white text-[22px] sm:text-[32px] md:text-[40px] lg:text-[40px] sm:leading-[48px] sx:leading-[28px] mt-4' data-aos="fade-left">Showcasing our <span className='text-themedark'> latest achievements </span> during industry gatherings</h3>
                      <p className='mt-5 text-gray-700 dark:text-[#f0f0f0]' data-aos="fade-left">Throughout the year, we engage with the AI community via a wide range of virtual, in-person, and hybrid events—participating as sponsors or hosting these gatherings ourselves. At these forums, our engineers and researchers highlight our newest product innovations, research achievements, and publications.</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <section className='bg-white dark:bg-black pt-[90px] overflow-hidden transition-colors duration-300' >
        <div className="relative  overflow-hidden py-[100px] ">
          ...
          <div className="relative z-10  px-5 pointer-events-none">
            <div className="text-center text-black dark:text-white">
              <h3 className="text-black dark:text-white text-[22px] sm:text-[32px] md:text-[40px] lg:text-[40px] sm:leading-[48px] leading-[28px] " data-aos="zoom-in">
                Looking to keep up with  <br />everything <span className='text-themedark'>AI</span> at Meta?
              </h3>
              <p className="mt-6 sm:text-[16px] text-[14px] text-gray-700 dark:text-[#f0f0f0]" data-aos="zoom-in">
