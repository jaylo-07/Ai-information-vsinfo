...
const Sidebar = () => {
  ...
  const { threads, recentPrompt, theme, isMobileSidebarOpen, currentThreadId, isTemporaryChat } = useSelector((state) => state.chat);
  const authState = useSelector((state) => state.auth);

  useEffect(() => {
    // Fetch threads when Sidebar mounts, whether logged in or not
  ...
  return (
    <>
      ...
      <div className={`
        ...
        flex flex-col justify-between bg-white dark:bg-[#131314] lg:bg-transparent dark:lg:bg-transparent lg:border-r border-black/5 dark:border-white/5 p-4
        ...
        <div>
          ...
          <div className={`flex gap-1 mt-[40px] ${extended ? 'w-full' : 'w-10 flex-col gap-2'}`}>
            <div onClick={() => {
              dispatch(newChat());
              dispatch(setIsMobileSidebarOpen(false));
            }} className={`rounded-full cursor-pointer transition-colors group bg-black/5 dark:bg-[#1e1e1e] text-gray-700 dark:text-[#a0a09e] hover:bg-black/10 dark:hover:bg-[#252525] dark:hover:text-white ${extended ? 'w-full flex items-center gap-3 p-2.5 pl-4' : 'flex items-center justify-center w-10 h-10 px-0'}`}>
              <Plus className="w-5 h-5 shrink-0 text-gray-900 dark:text-white/80" />
              {extended && <p className="font-medium whitespace-nowrap text-[14px] text-gray-900 dark:text-white/80">New chat</p>}
            </div>
            ...
            <div
              title="Temporary chat"
              onClick={() => {
                dispatch(setIsTemporaryChat(!isTemporaryChat));
                dispatch(setIsMobileSidebarOpen(false));
              }}
              className={`w-10 h-10 flex items-center justify-center flex-shrink-0 cursor-pointer rounded-full transition-colors ${isTemporaryChat ? 'bg-gray-300 dark:bg-white/20' : 'bg-black/5 dark:bg-[#1e1e1e] hover:bg-black/10 dark:hover:bg-[#252525]'}`}
            >
            ...
            <div className="flex flex-col mt-8 animate-fadeIn">
              ...
              <div className="flex flex-col gap-1 max-h-[50vh] overflow-y-auto scrollbar-hidden">
                ...
                {threads && threads.map((item, index) => {
                  ...
                  return (
                    <div key={item.id} onClick={(e) => {e.stopPropagation(); loadPrompt(item.id); dispatch(setIsMobileSidebarOpen(false));}} className={`relative flex items-center justify-between gap-2 p-2 pl-4 mb-[2px] rounded-full cursor-pointer transition-colors group ${showChatMenu === index || isActive ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300' : 'bg-black/5 dark:bg-[#1e1e1e] text-gray-700 dark:text-[#a0a09e] hover:bg-black/10 dark:hover:bg-[#252525] hover:text-gray-900 dark:hover:text-white'}`}>
