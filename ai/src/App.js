  ...
  return (
    <div className="min-h-screen flex bg-[#131314] text-white">
      ...
      <div className="flex-1 flex flex-col bg-[#131314]">
        <Header />
        <main className="flex-1 flex items-end justify-center px-4 lg:px-12 pb-10 lg:pb-16">
          <div className="w-full max-w-3xl mx-auto">
            <div className="flex items-center gap-2 text-sm md:text-base text-[#c4c7c5] mb-1">
              <span className="text-lg">✨</span>
              <p>Hi umang jogani</p>
            </div>
            <h1 className="text-[1.8rem] md:text-3xl lg:text-4xl font-semibold tracking-tight bg-gradient-to-r from-white via-white to-[#9ca3af] bg-clip-text text-transparent">Where should we start?</h1>
            ...
            <div className="mt-6 md:mt-8">
              <div ref={menuRootRef} className="bg-[#1f1f20] rounded-3xl px-4 py-3 md:px-6 md:py-4 border border-white/10 shadow-[0_0_0_1px_rgba(255,255,255,0.04)] flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between text-xs md:text-sm text-[#c4c7c5]">
                  <div className="flex items-center gap-1.5">
                    <div className="relative">
                      <button ref={plusButtonRef} type="button"
                        ...
                        className={`inline-flex items-center justify-center w-8 h-8 md:w-9 md:h-9 rounded-full hover:bg-white/10 transition-colors ${openMenu === 'plus' ? 'bg-white/10 text-white' : ''}`}
                        title="Add"
                      >
                      ...
                      {openMenu === 'plus' && (
                        <div role="menu" aria-label="Add menu" className="absolute top-full left-0 mt-2 z-50 bg-[#1e1f20] border border-white/10 rounded-2xl py-2 w-[220px] shadow-2xl">
                          {plusMenuItems.map((item) => (
                            <button key={item.label} type="button" role="menuitem" className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-white/10 text-white/90 hover:text-white transition-colors text-left" onClick={() => setOpenMenu(null)}>
                    ...
                    <div className="relative">
                      <button ref={toolsButtonRef} type="button"
                        ...
                        className={`inline-flex items-center gap-2 text-[11px] md:text-xs hover:text-white transition-colors rounded-full px-2 py-1.5 ${openMenu === 'tools' ? 'bg-white/10 text-white' : ''}`}
                      >
                      ...
                      {openMenu === 'tools' && (
                        <div role="menu" aria-label="Tools menu" className="absolute top-full left-0 mt-2 z-50 bg-[#1e1f20] border border-white/10 rounded-2xl py-2 w-[240px] shadow-2xl">
                  ...
                  <div className="flex items-center gap-3">
                    <button className="hidden sm:inline-flex items-center gap-1 md:gap-1.5 text-[11px] md:text-xs hover:text-white transition-colors">
                      <span>Thinking</span>
                      <ChevronDown className="w-3 h-3" />
                    </button>
                    <button className="flex items-center justify-center w-8 h-8 md:w-9 md:h-9 rounded-full bg-white text-black hover:bg-zinc-200 transition-colors">
